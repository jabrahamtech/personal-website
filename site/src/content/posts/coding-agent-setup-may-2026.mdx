---
title: "Stop running one coding agent for everything (my May 2026 setup)"
...
- [Why layered codebases punish humans and AI coding agents (and what vertical slice fixes)](/posts/from-mvc-to-vertical-slice): structure the repo so a bounded specialist can land a slice without tracing the whole stack first.
- [Why most AI agent programs are repeating RPA's mistakes](/posts/rpa-already-solved-this): the operating-model view of the same written-down, least-privilege discipline, scaled past a single developer's setup.
- [Most "human-in-the-loop" is escalation done badly](/posts/most-hitl-is-escalation-done-badly): stop the specialist, hand the failure back to the planner, re-handoff only once the assumptions are clear.
