...
- [When your AI agent passes the demo and fails the audit](/posts/agent-governance-au-insurtech) — the same governance and audit primitives, mapped onto Australian insurance supervision.
- [Most "human-in-the-loop" is escalation done badly](/posts/most-hitl-is-escalation-done-badly) — one of the four practices in close-up: designing human involvement deliberately instead of arguing it from first principles.
- [Stop running one coding agent for everything](/posts/coding-agent-setup-may-2026) — the same least-privilege discipline applied to my own coding setup: written roles, scoped tools, skills as workflows rather than secret stores.
