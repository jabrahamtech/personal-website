...
<Figure
  ...
  alt="A deterministic outer loop with three nodes, open step (LLM), captured value, next step, plus four back-channel buttons beneath: pause, previous, next, confirm."
...
Related, but a different angle. That post (coming) is about the *back-end* parts of an agent that should stay deterministic: rules, queues, database constraints, retries. This one is about the *front-end*: what the user clicks, sees, and steers with.
...
- [Most "human-in-the-loop" is escalation done badly](/posts/most-hitl-is-escalation-done-badly): once the model owns a step, on what trigger a human gets pulled back in.
- [What Polymarket taught me about event-driven](/posts/polymarket-bot-event-driven): the same line drawn on the back end, which work gets to be loose and which has to be synchronous.
