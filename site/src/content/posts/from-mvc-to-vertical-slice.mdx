...
- [The boring infrastructure stack I'd actually pick for a startup](/posts/boring-infrastructure-stack-for-startups) — the infrastructure counterpart: the boring, debuggable stack that pairs with a boring, navigable codebase.
- [Stop running one coding agent for everything](/posts/coding-agent-setup-may-2026) — the agent-setup side of the same idea: once the repo is navigable, hand a slice to a bounded specialist instead of overloading one agent with the whole stack.
