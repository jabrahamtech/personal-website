...
```
book_update  →  strategy  →  orders  →  executor  →  risk  →  backend.submit  →  fills
                                                                                  ↓
                                                                              event_log (SQLite)
                                                                                  ↑
...
class CoalescingBuffer:
    """Queue-compatible buffer that keeps only the latest value per key."""
...
risk_result = await self.risk.check(orders)            # direct call, not a topic
if not risk_result.passed:
    await self._reject_orders(orders, f"risk: {risk_result.reason}")
    return []

self._backend.commit_capital(total_cost)               # sync reservation
fills = await self._backend.submit_orders(orders)      # only real awaitable

for fill in fills:
    self.positions.on_fill(fill, ...)                  # sync position update
```
...
def register_fill(self, fill: Fill, order: OrderRequest) -> None:
    ...
    if buffered:                                    # venue beat us
        ...
        self._pending[fill.clob_order_id] = (fill, order, self._clock.now())
```
