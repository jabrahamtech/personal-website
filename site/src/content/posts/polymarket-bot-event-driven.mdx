...
The codebase that shipped is a hybrid: an async bus for facts, a synchronous hot path for decisions, an append-only SQLite log off to the side for memory. None of the three is optional. None of them lives inside a single pattern. The literature has been saying this since at least 2011 (LMAX) and 2017 (Fowler). The 2025 vendor pitch — Kafka shape for everything — quietly drops the middle layer.
...
A production trading bot should be event-driven at the edges and synchronous at the decision point, with durable memory kept separate.

The shape that survived in this Polymarket bot is the three-layer hybrid: facts, decisions, memory. Facts move through the async bus: order-book updates, user fills, boundary events, the stream of things that happened or changed. Decisions run through the executor's hot path: strategy intent, risk checks, capital reservation, backend submit, position update. Memory sits off to the side as an append-only event log of decisions, fills, errors, and reconciliations.
...
The rule is simple: stream the facts, synchronise the decision, append the memory.
...
**Three-layer hybrid**
The architecture that separates facts, decisions, and memory instead of forcing them into one event-driven pattern. Facts stream through the bus, decisions run synchronously in the hot path, and memory is stored as an append-only record.

**Synchronous decision hot path**
The executor-owned path where strategy intent becomes an order: receive command, run risk, reserve capital, submit to the backend, update position. This path is synchronous because correctness matters more than loose coupling at the moment of action.

**Append-only event log as memory**
A durable record of decisions, fills, errors, and reconciliations that exists beside the bus, not inside it. The log is for explanation, audit, and replay; the bus is for moving live facts through the process.

**Supervisor + circuit breaker pattern**
The runtime pattern that keeps long-lived engines alive without pretending crashes are harmless. Supervised tasks restart with backoff, and the circuit breaker trips when repeated failures show the system is no longer healthy.

**Boundary price service**
The service that turns time and market boundaries into typed events the rest of the bot can consume. It keeps resolution and redemption logic driven by explicit boundary signals instead of scattered timers.
