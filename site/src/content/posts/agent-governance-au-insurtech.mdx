...
{/* TODO (structure): this post does two jobs, a practical whiteboard test and a
    full AU-insurance regulatory explainer. If it grows further, split the
    regulatory mapping (the AU map table plus the programme and per-decision
    primitive citations) into its own companion post and keep this one on the
    whiteboard test. */}
...
<Figure
  ...
  caption="Fig. 1: claims-triage flow with the six per-decision primitives labelled at the architectural points where they live."
/>
...
<Figure
  ...
  caption="Fig. 2: bolt-on governance versus designed-in governance, with the trace pipeline as the structural difference."
/>
...
- [Databricks: AI Security Framework v3.0 (DASF), concrete implementation of the security pillar referenced above](https://www.databricks.com/blog/agentic-ai-security-new-risks-and-controls-databricks-ai-security-framework-dasf-v30)
...
- [Why most AI agent programs are repeating RPA's mistakes](/posts/rpa-already-solved-this): the governance playbook agent teams keep reinventing from scratch.
- [Most "human-in-the-loop" is escalation done badly](/posts/most-hitl-is-escalation-done-badly): where a human re-enters the loop, and on what trigger an auditor expects to see it.
