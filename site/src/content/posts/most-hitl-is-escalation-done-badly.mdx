...
- [When to force the LLM, and when to use a button](/posts/when-to-force-the-llm-and-when-to-use-a-button): which parts of a workflow the model should own at all.
- [What Polymarket taught me about event-driven](/posts/polymarket-bot-event-driven): the architecture side of the same discipline, which decisions get to be loose and which have to be deterministic.
- [When your AI agent passes the demo and fails the audit](/posts/agent-governance-au-insurtech): what an auditor expects to see when a human steps in.
- [Why most AI agent programs are repeating RPA's mistakes](/posts/rpa-already-solved-this): human involvement designed in, not argued from first principles.
- [Stop running one coding agent for everything](/posts/coding-agent-setup-may-2026): when a Codex run goes wrong, stop it and hand the failure back to the planner rather than letting it retry.
